...
스킬을 사용자처럼 처음부터 따라 돌려서 실제 챕터 하나를 만들었다. 주제는
**`git`의 스테이징 영역과 커밋** — 견본(자바스크립트 이벤트 루프)과 다른 주제로
잡아 문서가 견본에 기대는지 드러나게 했다. 산출물은 저장소 밖 스크래치패드에 두고
커밋하지 않았다.
...
| 재는 것 | 값 | 목표 |
|---|---|---|
| 커리큘럼 | 챕터 4개, 목표 5·5·4·4개 | 챕터 4개 안팎, 목표 4~6개 |
| 1단계 산출물 | `curriculum.md` 하나, **HTML 0개** | HTML 없음 |
| 1장 슬라이드 | **60장** | 60~70장 |
| 추정 학습 시간 | **70분** | 60~120분 |
| `__deckCheck()` | **`[]` (위반 0건)** | 0건 |
| 목표별 산문 | 2,914~3,396자 (하한 1,200) | 하한 이상 |
| 열기 장 산문 | 795자 (하한 400) | 하한 이상 |

위반은 세 번의 반복으로 0이 됐다. 두 번째 회차에서 나온 4건은 `.steps` 5단계
슬라이드 세 장의 세로 넘침(17·65·17px)과 목표 3의 퀴즈 누락이었다. 넘침은 문서대로
글을 깎지 않고 장을 나눠 고쳤다.

### 스킬이 제대로 작동한 것

- 갈림길이 정확히 갈렸다. `curriculum.md`가 없으니 커리큘럼만 쓰고 A4에서 멈췄고,
  1단계에서 HTML이 만들어지지 않았다.
- 게이트 메시지만 보고 무엇을 고칠지 알 수 있었다. 세 번의 반복에서 메시지를 못
  알아본 경우가 없었다.
- 비유를 다섯 목표 중 하나에만 썼다. "억지면 빼라"가 이 주제에서 실제로 작동했다 —
  `diff`의 좌변·우변에는 맞는 비유가 없다.

### 고친 것 (실제로 막혔던 것만)

| 파일 | 무엇 | 왜 |
|---|---|---|
| `references/chapter-anatomy.md` 머리말 | 레이아웃·구성요소 클래스 목록의 주인이 뼈대 CSS라는 포인터 | 슬라이드를 쓰는 데 필요한 클래스(`.split` `.stack` `.figure` …) 목록이 어느 문서에도 없어서 뼈대 CSS를 직접 읽어 목록을 만들어야 했다 |
| `references/chapter-anatomy.md` 4.5 | `.steps`를 두 장으로 나눌 때 번호 이어 붙이는 방법 | 넘침 처방대로 장을 나누면 번호가 1로 돌아가 독자가 4단계를 1단계로 읽는다. `<ol start>`는 CSS 카운터에 무시된다 |
| `SKILL.md` 닫기 장 | 퀴즈를 목표 끝에 둘 때 담는 장이 무엇인지 | "목표 끝마다 퀴즈"를 권하는데 `closing`에는 `data-goal`을 못 붙여서, 어느 장에 넣어야 하는지 판단할 근거가 없었다 |
| `references/curriculum-design.md` 4절 | 학습 시간 공식을 목표 하나당 산문 규모로 역산 | 목표별 하한(1,200자)만 채우면 30분에 끝난다. 목표 구간에 맞추려면 목표당 3,000자 규모라는 것을 직접 역산해야 했다 |

규칙을 두 번째 파일에 옮겨 적지 않았다 — 클래스는 목록 복사 대신 포인터로, 산문
규모는 하한 값을 다시 적지 않고 "하한은 통과선"이라고만 했다.

### 고치지 않고 보고만 한 것

- **`assets/deck.html`의 자기검사 주석이 낡았다** (855~860줄). "학습 연결 게이트는
  뒤 작업에서 붙는다 … 하한 둘도 뒤 작업에서 붙는다"고 적혀 있지만 바로 아래에 다
  구현돼 있다. 처음 읽고 "목표 하한은 아직 없구나"로 오해했다. 뼈대와 검사기는
  손대지 않기로 했으므로 세 줄 삭제는 결정에 맡긴다.
- **`curriculum-design.md` 부록의 완성 예시가 하필 git 과정이다.** 그 1장 목표가 이
  검증 주제와 도메인이 겹쳐서, 커리큘럼 갈래에서는 "견본에 기대는지"를 재는 힘이
  약했다. 부록 목표를 쓰지 않고 도달점에서 새로 뽑아 우회했다. 예시를 바꾸면 시험
  조건은 깨끗해지지만 문서가 나아지지는 않아서 그대로 뒀다.

### 회귀 확인

| 확인 | 결과 |
|---|---|
| `node tests/build.mjs broken` 후 `tests/out/broken.html` | **14건** — `tests/README.md`의 표와 메시지·순서까지 일치 |
| `assets/deck.html` | **`[]`** |
| `__deckStats` (fixture) | `{minutes:1, prose:40, codeLines:1, figures:0, quizzes:1}` — README와 일치 |

전체 보고서는 `.superpowers/sdd/2026-07-29-concept-slides-curriculum/task-17-report.md`.

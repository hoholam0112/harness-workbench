...
## LLM wiki
LLM wiki에 대한 절차는 `experiment-wiki` 스킬로 분리한다. references로 나누지 않고
SKILL.md 안에 내용을 직접 담는다.
...
## 실험보고서
실험보고서를 html로 작성하는 절차는 `experiment-report` 스킬로 분리한다. references로
나누지 않고 SKILL.md 안에 내용을 직접 담는다.
